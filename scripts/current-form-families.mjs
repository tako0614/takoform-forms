...
function main() {
  ...
  if (process.argv.length !== 3 || !["--write", "--check"].includes(mode)) {
    throw new Error(
      "usage: bun scripts/current-form-families.mjs --write|--check",
    );
  }
...
function writeContractCandidates({
  ...
  for (const contract of contracts) {
    ...
    if (
      ...
      throw new Error(
        `${listKey} source contains a duplicate or invalid contract identity`,
      );
    }
    ...
    if (typeof definitionRaw !== "string" || definitionRaw.length === 0) {
      throw new Error(
        `${name}: source renderer emitted no definitionJson text`,
      );
    }
    ...
    if (schemaDigest !== contract.schemaDigest) {
      throw new Error(
        `${name}: rendered schemaDigest drifted from the catalog`,
      );
    }
...
function renderFamilySources() {
  ...
  if (result.status !== 0) {
    throw new Error(
      `family Form source rendering failed\n${result.stderr}${result.stdout}`,
    );
  }
  ...
  for (const family of rendered.families) {
    if (
      ...
      throw new Error(
        "family Form source emitted an invalid family metadata record",
      );
    }
    ...
    for (const form of family.forms) {
      if (
        ...
        throw new Error(
          `${family.group}: family Form source contains an invalid or duplicate Form`,
        );
      }
...
export function validatePublisherPathMetadata(source) {
  ...
  for (const family of source.families) {
    if (
      ...
      throw new Error(
        `unsafe official family group ${JSON.stringify(family?.group)}`,
      );
    }
    for (const form of family.forms) {
      if (
      ...
      for (const name of Object.keys(form.fixtures)) {
        if (name.length > 127 || !FIXTURE_NAME.test(name)) {
          throw new Error(
            `${family.group}/${form.slug}: unsafe fixture name ${JSON.stringify(name)}`,
          );
        }
      }
    }
...
function compareTrees(expectedRoot, actualRoot, label) {
  if (!existsSync(actualRoot)) {
    throw new Error(
      `tracked ${label} candidate set is missing; run bun run build:current-form-families`,
    );
  }
...
function compareFile(expected, actualPath, label) {
  if (
    ...
    readFileSync(actualPath, "utf8") !== expected
  ) {
    throw new Error(
      `tracked ${label} is stale; run bun run build:current-form-families`,
    );
  }
}
  ...
  const visit = (directory, prefix = "") => {
    for (const name of readdirSync(directory).sort()) {
      ...
      if (info.isSymbolicLink())
        throw new Error(`${relative}: symlink is forbidden`);
      if (info.isDirectory()) visit(full, relative);
      else if (info.isFile())
        entries.push([relative, digest(readFileSync(full))]);
...
function digestCanonicalJSON(file) {
  const result = spawnSync(
    "go",
    ["run", "./cmd/form-package", "digest", file],
    {
      cwd: repositoryRoot,
      encoding: "utf8",
    },
